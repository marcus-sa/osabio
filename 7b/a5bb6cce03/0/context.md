...
Stop hook feedback:
Prompt hook condition was not met: This hook requires the Brain MCP tools (log_observation, create_provisional_decision, etc.) to be available for logging session work. The current environment does not have these tools loaded. Please ensure the Brain MCP integration is available before running this hook.
