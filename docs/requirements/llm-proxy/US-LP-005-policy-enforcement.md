...
A new workspace has no model access policies defined. The proxy defaults to allowing all models (permissive by default). A warning observation is created: "Workspace brain-v1 has no LLM model access policies. All models are accessible to all agent types."
...
## Technical Notes
- Query Brain's existing policy graph -- do not create a separate policy system
...
- Agent type determination: from custom header (X-Brain-Agent-Type) or inferred from metadata patterns
...
- Brain policy engine (existing -- policies managed via policy CRUD UI)
