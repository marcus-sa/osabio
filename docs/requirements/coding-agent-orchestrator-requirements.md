...
Enable users to assign engineering tasks to a coding agent (via OpenCode SDK) directly from the Osabio UI. The agent reads project context via Osabio MCP, implements changes, and reports back. Focus: self-hosted OpenCode SDK integration. Cloud (Modal sandboxes) deferred.
...
2. **MCP integration** — opencode agent calls existing Osabio MCP tools (already implemented)
...
| FR-1.3 | System creates an opencode session configured with Osabio MCP server | Must |
...
| FR-2.2 | Key events (file change, tool call, error, completion) forwarded to Osabio SSE | Must |
...
| NFR-2 | SSE event forwarding latency < 1s from opencode → Osabio UI | Should |
...
- **Agent event bridge** — subscribes to opencode events, forwards to Osabio SSE
