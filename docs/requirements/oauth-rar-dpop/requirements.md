...
2. **Custom Authorization Server (Osabio Operations)**: Full RAR + Full DPoP for ANY actor (human or agent) performing ANY operation on the knowledge graph. No scopes. No tiering. Every operation is a structured `osabio_action` intent.
3. **The Bridge**: When a human needs to touch the Osabio, they exchange their Better Auth session for a DPoP-bound RAR token from the Custom AS.

## Business Rules

### BR-1: Uniform Authorization Model (No Tiering)

- ALL Osabio operations -- reads, writes, integrations -- require RAR authorization with structured `osabio_action` authorization_details
...
- Every RAR access token MUST reference a valid, authorized intent via `urn:osabio:intent_id` claim
- The token's `authorization_details` MUST match the intent's `osabio_action` (or its human-constrained variant)
...
- Only the holder of the matching private key can present the token to the Osabio
...
- The Osabio resource server maintains a time-windowed nonce cache (entries auto-expire after clock skew window)
- Reused `jti` values are rejected with a specific error code
- The nonce cache MUST be scoped per Osabio resource server instance (injected via dependency injection, not module-level singleton)
...
- Better Auth sessions CANNOT directly access Osabio resources
- Session cookies presented to the Osabio resource server are rejected with 401 "dpop_required"
- Bearer tokens presented to the Osabio resource server are rejected with 401 "dpop_required"
...
- Better Auth scopes (e.g., "dashboard:access") are consumed by the dashboard UI only, never by the Osabio

### BR-7: Human Parity

- Human and agent tokens are verified identically at the Osabio boundary
- The Osabio resource server does not differentiate verification logic based on actor type
- Audit trail entries use the same `osabio_action` format for both human and agent operations
...
- ALL Osabio operations require intent submission -- reads, writes, integrations
...
- New grant type: `urn:osabio:intent-authorization`
...
- Verifies authorization_details matches intent osabio_action
- Verifies DPoP proof key thumbprint matches intent dpop_jwk_thumbprint
- Issues DPoP-bound access token with `cnf.jkt`, `authorization_details`, `urn:osabio:intent_id` claims
...
- Authorizer Agent evaluates the osabio_action (same pipeline as agent intents)
- Issues DPoP-bound access token with osabio_action authorization_details
...
### FR-6: DPoP Proof Verification at Osabio Resource Server
...
### FR-7: RAR Operation Scope Verification at Osabio Resource Server

- Extract requested operation from API route and request body as a `osabio_action`
...
- Every intent submission, evaluation, routing decision, human consent action, token issuance, and Osabio resource server verification is logged
...
- Human and agent operations use the same audit format -- `osabio_action` entries
...
- DPoP proof verification at Osabio resource server: < 10ms per request
...
- Session cookies cannot access the Osabio (enforced at Osabio boundary)
- No scope fallback at the Osabio (enforced at Osabio boundary)
...
| SurrealDB (existing) | Available | Schema extension for DPoP binding fields, osabio_action types |
...
|---|---|---|---|
| Clock skew between actor runtimes and Osabio resource server | Medium | Medium | Configurable tolerance window, NTP requirement, actionable error messages |
