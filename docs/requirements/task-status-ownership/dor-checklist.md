...
| 3 | Dependencies identified | Pass | Depends on existing commit-task-refs.ts, Brain HTTP API, git hook infrastructure |
...
| 8 | Shared artifacts identified | Pass | extractReferencedTaskIds (reused), update_task_status MCP tool (reused), Brain HTTP client (reused) |

## Implementation Order (suggested)

1. **US-1 + US-2** (remove server-side transitions) — unblocks everything, smallest risk
2. **US-3** (brain commit-check command) — core new functionality
