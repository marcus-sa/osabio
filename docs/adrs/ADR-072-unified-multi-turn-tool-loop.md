...
The proxy's tool use interception loop (Step 8.5 in `anthropic-proxy-route.ts`) handles brain-native and integration tool calls in separate branches within each iteration. When the LLM response contains `tool_use` blocks, the loop classifies them and enters either the brain-native branch or the integration branch -- but not both.

The Anthropic Messages API allows multiple `tool_use` blocks per response. An LLM can call `search_entities` (brain-native) and `github.create_issue` (integration) in the same response. The current two-branch design drops one category in this scenario: if `hasBrainNative` is true, the integration branch is skipped entirely.
...
2. Classify all blocks (brain-native, integration, unknown)
3. If all unknown, break (pass through to runtime)
4. Execute brain-native calls via `executeBrainNativeTools` (existing)
...
- **What**: Run brain-native branch first, then integration branch, in two sub-steps per iteration.
...
- **What**: If a response contains both brain-native and integration tool calls, return an error tool_result for all of them.
- **Expected impact**: Simplest implementation.
- **Why rejected**: Artificially limits LLM behavior. The LLM reasonably calls tools from different sources when solving a complex task (e.g., "search Brain's graph for the task, then create a GitHub issue"). Rejecting this forces the LLM into unnecessary sequential behavior.

## Consequences
- **Positive**: Handles all tool call combinations (brain-native only, integration only, mixed, unknown)
...
- **Negative**: Brain-native and integration tools execute sequentially within an iteration (could be parallelized later if latency is an issue)
