...
Brain's MCP Tool Registry requires authentication when connecting to remote MCP servers. The current design assumes manual OAuth2 provider configuration (admin enters authorization_url, token_url, client_id, client_secret). This doesn't match reality:
...
**Static headers** are a Brain-specific convenience (not part of MCP spec). Header values are encrypted at rest with AES-256-GCM (ADR-066).
...
- Client ID Metadata Documents as primary registration (Brain hosts `/.well-known/oauth-client-id`)
...
- Brain serves `/.well-known/oauth-client-id` as a public endpoint
