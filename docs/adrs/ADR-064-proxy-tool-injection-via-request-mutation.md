...
## Context
The MCP Tool Registry (#178) needs to inject Brain-managed tool definitions into LLM requests. The proxy already mutates request bodies for context injection (ADR-046). We need to decide how tool definitions are delivered to the LLM.
...
## Decision
Inject Brain-managed tools by appending to the `tools[]` array in the parsed request body, following the same body mutation pattern as context injection (step 7 in the proxy pipeline). This becomes step 7.5, after context injection and before forwarding.

Runtime tools occupy the front of the array (original positions preserved). Brain-managed tools are appended at the end. If a Brain-managed tool name collides with a runtime tool name, the Brain tool is skipped (runtime takes precedence).

## Alternatives Considered

### Alternative 1: Separate tools-only request header
- **What**: Send Brain tools in a custom `X-Brain-Tools` header, rely on the LLM provider to merge them.
...
- **What**: Brain exposes tools via its existing CLI MCP server (stdio), agents discover them via MCP protocol.
...
- **What**: Describe available tools in the system prompt `<brain-context>` block, relying on the LLM to "call" them via structured text output.
