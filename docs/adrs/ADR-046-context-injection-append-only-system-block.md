...
## Context
The LLM proxy needs to inject Brain knowledge graph context (decisions, learnings, observations) into LLM requests so that coding agents benefit from organizational knowledge without requiring MCP integration or client-side setup.
...
## Decision
Append a new system content block AFTER all existing blocks. Normalize the `system` field to array form if it arrives as a string. Mark the appended block with `cache_control: { type: "ephemeral" }`. Wrap content in a `<brain-context>` XML tag with workspace/session metadata attributes.

## Alternatives Considered

### Alternative 1: Modify existing system blocks (inline injection)
- **What**: Insert Brain context into the middle or beginning of the existing system prompt.
...
- **What**: Insert a synthetic `user` message at the beginning of the conversation containing Brain context.
...
- **Positive**: Clear separation -- `<brain-context>` XML wrapper makes the injected block identifiable and parseable
