# Research: Clawith Six Trigger Types -- Gap Analysis for Brain
...
Brain's current event/hook architecture covers reactive triggers (SurrealDB DEFINE EVENT webhooks, Claude Code hooks) and on-demand scanning (Observer). It lacks proactive, time-based agent activation entirely. Three of Clawith's six trigger types (cron, interval, poll) would fill genuine gaps in Brain's architecture. Two others (webhook, on_message) partially overlap with existing capabilities but offer a more unified model. One (once) is a minor variant of cron with limited standalone value.
...
**Analysis**: This is architecturally significant. The coupling between triggers and working memory prevents orphaned triggers and gives agents lifecycle-aware scheduling. Brain has an analogous concept: Intents (structured action requests) and Tasks (work items with status). A Brain adaptation could bind triggers to Tasks or Intents rather than introducing a new "Focus Item" entity.

---

### Finding 3: Brain's Current Event Architecture Is Reactive-Only

Brain's existing trigger/event mechanisms:
...
**Analysis**: All Brain triggers are reactive -- they fire in response to something that already happened (a record mutation, a session event, a human action). There is no mechanism for Brain to proactively wake an agent at a future time, monitor an external endpoint, or fire on a schedule. The Observer scan endpoint exists but requires external invocation -- there is no built-in scheduler.

---

## Gap Analysis: Clawith Triggers vs. Brain Capabilities

### Trigger-by-Trigger Assessment

#### 1. cron (Recurring Schedule) -- ADOPT

**Brain gap**: Critical. Brain has no built-in scheduler. The Observer requires manual API calls or external cron to run scans. Native agents (per the brain-native-agent-runtime research) will need scheduled activation for maintenance tasks, report generation, and proactive monitoring.
...
**Implementation complexity**: Medium. Requires a scheduler service (e.g., `croner` library for cron parsing), a `trigger` table in SurrealDB to persist job definitions, and a runtime loop that evaluates pending triggers. The OpenClaw implementation stores jobs under `~/.openclaw/cron/` -- Brain would store them as graph nodes.
...
**Brain gap**: Minimal. Brain is a knowledge graph coordinator, not an infrastructure monitoring tool. HTTP endpoint monitoring is the domain of uptime monitors (Grafana, Datadog, Prometheus) which can feed results into Brain via webhooks.

**Value**: Low for Brain's use case. Brain agents reason about knowledge, decisions, and coordination -- not HTTP response codes.
...
**Brain gap**: Partial. Brain already has a `webhook/` route domain for GitHub webhook integration. However, this is hardcoded to GitHub events. There is no general-purpose webhook ingestion that converts arbitrary external events into graph entities or agent activations.

**Value**: Medium-High. A general webhook endpoint would allow CI/CD systems, monitoring tools, Slack, and other services to inject events into Brain's graph, potentially triggering agent responses.

**Implementation complexity**: Low-Medium. Brain already has the HTTP infrastructure and webhook routing for GitHub. Generalizing this to a configurable webhook registry (URL path -> handler -> graph entity creation + optional agent activation) is incremental work.

**Recommendation**: **Adapt**. Generalize the existing GitHub webhook infrastructure into a configurable webhook registry. Each registered webhook path maps to: (1) a graph entity creation template (what to write to the graph), and (2) an optional trigger activation (which agent to wake). This extends Brain's existing pattern rather than introducing a new system.

---

### Summary Matrix

| Trigger Type | Brain Gap | Value | Complexity | Recommendation |
|-------------|-----------|-------|------------|----------------|
...
Rather than replicating Clawith's trigger system directly, Brain should implement triggers as first-class graph nodes:
...
Clawith's Focus-Trigger Binding maps cleanly to Brain's existing entities:

| Clawith Concept | Brain Equivalent |
|----------------|-----------------|
...
- **Cost amplification**: Cron triggers invoking LLM calls on a schedule can accumulate cost rapidly. Mitigate with: spend budget integration (Brain already has proxy spend tracking), trigger cost estimation before creation.
...
| Brain native runtime research | local | High (1.0) | Internal | 2026-03-22 | N/A (internal) |
| Brain SurrealDB schema | local | High (1.0) | Internal | 2026-03-22 | N/A (internal) |
| Brain AGENTS.md / README | local | High (1.0) | Internal | 2026-03-22 | N/A (internal) |
...
**Recommendation**: Review Clawith source code or OpenClaw Gateway documentation for failure handling patterns before implementing Brain's trigger system.
...
**Recommendation**: Design Brain's trigger scheduler with explicit concurrency limits from the start and benchmark before production use.

---

## Recommendations for Further Research

1. **Scheduler library evaluation**: Compare `croner` (used by OpenClaw), `node-cron`, and Bun-native timer approaches for Brain's runtime.
...
3. **Agent self-scheduling authority model**: Define what authority scopes are needed for agents to create their own triggers safely. This intersects with Brain's existing policy and intent systems.
...
[5] Brain project. "Brain as Native Agent Runtime (internal research)". Local. 2026-03-21. docs/research/brain-native-agent-runtime.md.
[6] Brain project. "SurrealDB schema (surreal-schema.surql)". Local. 2026. schema/surreal-schema.surql.
[7] Brain project. "README.md and AGENTS.md". Local. 2026. README.md, AGENTS.md.
...
- **Sources Cited**: 7
- **Cross-References Performed**: 4 (trigger type definitions, focus-trigger binding, Brain event architecture, implementation patterns)
