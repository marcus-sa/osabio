...
**Source Selection**: Types: official (IETF RFCs), technical_docs (MCP spec), codebase (primary implementation) | Reputation: high | Verification: cross-referencing RFC semantics against existing Osabio types
...
| `authorization_details` | `OsabioAction[]` | RFC 9396 authorization details |
...
### Finding 2: ActionSpec-to-OsabioAction Mapping for MCP Tools

**Evidence**: Osabio's `OsabioAction` type implements RFC 9396's `authorization_details` element structure, adapted for the Osabio domain.
...
RFC 9396 defines `authorization_details` as an array of objects with a required `type` field and optional common fields (`actions`, `locations`, `datatypes`, `identifier`, `privileges`). Osabio's `OsabioAction` adapts this:
...
// Osabio's OsabioAction (domain-specific adaptation)
{
  "type": "osabio_action",                // Fixed type for Osabio domain
...
For MCP tool gating, the mapping from an intent's `action_spec` to a `OsabioAction` for RAR would be:
...
// Derived OsabioAction for RAR (auth-facing)
{
  type: "osabio_action",
...
**Analysis**: The existing `ActionSpec -> OsabioAction` transformation is straightforward. The `provider` + `action` fields in `action_spec` compose into a hierarchical resource identifier in the `OsabioAction`. The `params` field selectively maps to `constraints` for fields that policies need to gate (e.g., financial amounts). Not all params become constraints -- only those with policy-relevant semantics.
...
**Evidence**: The token endpoint implements a custom OAuth grant type `urn:osabio:intent-authorization` that exchanges an authorized intent for a DPoP-bound access token carrying `authorization_details`.
...
Agent                    Token Endpoint              Intent DB
  ...
  |                           |   - cnf.jkt: thumbprint |
  |                           |   - authorization_details: [OsabioAction]
  |                           |   - urn:osabio:intent_id |
  |                           |   - urn:osabio:workspace |
  |                           |                         |
...
```typescript
{
  ...
  "urn:osabio:intent_id": "intent-uuid",
  "urn:osabio:workspace": "workspace-uuid",
  "urn:osabio:actor_type": "agent"
}
```
...
- **Intent binding**: The `urn:osabio:intent_id` claim links the token to a specific authorized intent, enabling audit trail from token usage back to intent creation, policy evaluation, and agent reasoning.
...
2. **Action derivation**: Maps the HTTP method + URL path to a `OsabioAction` via `deriveRequestedAction()` using the route-action map.
...
4. **Scope verification**: `verifyOperationScope()` checks that the requested `OsabioAction` matches one of the token's `authorization_details`, including constraint bounds.
...
**Evidence**: The existing MCP route handles Osabio's own MCP tools (context, decisions, tasks, etc.) but does not yet proxy to external MCP servers. The proxy layer for external tool execution is the primary gap.
...
**Confidence**: High
**Verification**: `app/src/server/proxy/anthropic-proxy-route.ts`, `docs/research/osabio-native-agent-runtime.md`

**Current state**: Osabio's MCP route (`mcp-route.ts`) handles graph operations (context, decisions, tasks, observations, intents) directly. These are Osabio-native tools. The route-action map in `route-action-map.ts` maps each MCP endpoint to a `OsabioAction` for RAR verification.
...
```
Agent                    Osabio MCP Tool Proxy         Upstream MCP Server (e.g., Stripe)
  |                           |                              |
  |-- tools/call ------------>|                              |
  |   + Authorization: DPoP   |                              |
  |   + DPoP proof            |                              |
...
**Analysis**: The MCP tool proxy is the key architectural piece that connects the existing intent/RAR system to external tool execution. The design reuses the existing `authenticateAndAuthorize` pipeline from `mcp-dpop-auth.ts` -- the same DPoP + RAR verification works for both Osabio-native MCP tools and external MCP tool proxying. The main new work is: (a) upstream MCP server connection management, (b) tool name-to-OsabioAction mapping, and (c) trace recording for external tool calls.
...
- **Option B: Composite intent** -- One intent authorizes a sequence of actions via multiple `OsabioAction` entries in `authorization_details`. The proxy checks each call against the array.
...
```
Support Agent                   Osabio Platform                              Stripe MCP Server
     |                               |                                           |
     | 1. CREATE INTENT              |                                           |
     |------------------------------>|                                           |
...
|--------|--------|------------|------|-------------|----------------|
| Osabio codebase (intent/*) | Local | High | Implementation | 2026-03-24 | Y |
| Osabio codebase (oauth/*) | Local | High | Implementation | 2026-03-24 | Y |
| Osabio codebase (policy/*) | Local | High | Implementation | 2026-03-24 | Y |
| Osabio codebase (mcp/*) | Local | High | Implementation | 2026-03-24 | Y |
| Osabio codebase (proxy/*) | Local | High | Implementation | 2026-03-24 | Y |
...
**Issue**: The exact mechanism for Osabio to connect to upstream MCP servers (stdio, HTTP+SSE, WebSocket) is not yet designed. The proxy needs a registry of MCP server endpoints and authentication credentials per workspace.
**Attempted**: Searched codebase for upstream MCP connection code; found only Osabio-native MCP tools. Reviewed `docs/research/osabio-native-agent-runtime.md` which discusses tools but not upstream MCP proxying.
...
### Gap 2: Tool Name to OsabioAction Mapping Registry
**Issue**: The current `route-action-map.ts` maps Osabio MCP routes to `OsabioAction`s. For external MCP tools, the mapping from tool names to `OsabioAction` resource identifiers needs a configurable registry rather than hardcoded routes.
**Attempted**: Reviewed `route-action-map.ts`; it uses regex patterns matching Osabio's URL structure. External MCP tools use `tools/call` with a `name` parameter, which is a different pattern.
**Recommendation**: Create a `tool_action_map` that maps `{provider}:{tool_name}` to `OsabioAction` templates. Populate from MCP server `tools/list` responses, with workspace admins able to override constraint mappings.
...
No significant conflicts found. The codebase implementation aligns cleanly with RFC 9396 and RFC 9449 patterns. The `OsabioAction` type is a purposeful simplification of RFC 9396's more general `authorization_details` structure, which is an acceptable domain-specific adaptation.
...
[4] Osabio Codebase. "Intent System". Local. `app/src/server/intent/`. Accessed 2026-03-24.
[5] Osabio Codebase. "OAuth RAR+DPoP Types". Local. `app/src/server/oauth/types.ts`. Accessed 2026-03-24.
[6] Osabio Codebase. "Token Endpoint". Local. `app/src/server/oauth/token-endpoint.ts`. Accessed 2026-03-24.
[7] Osabio Codebase. "Policy Gate". Local. `app/src/server/policy/policy-gate.ts`. Accessed 2026-03-24.
[8] Osabio Codebase. "MCP DPoP Auth". Local. `app/src/server/mcp/mcp-dpop-auth.ts`. Accessed 2026-03-24.
