 ...
 *
 * Milestone 3: osabio init Step 7 — .claude/settings.local.json management
 *
 ...
 *   - Re-running osabio init updates token in place
 *   - .gitignore verification and warning
 *   - ~/.osabio/config.json proxy_token storage
 *
 * These tests exercise the CLI config functions directly (unit-level driving port)
 * rather than the full `osabio init` flow, since the OAuth flow is tested elsewhere.
 *
...
beforeEach(() => {
  tmpDir = fs.mkdtempSync(path.join(os.tmpdir(), "osabio-proxy-test-"));
...
/**
 * Simulates the settings.local.json write logic from osabio init Step 7.
...
function writeSettingsLocal(
  ...
  env.ANTHROPIC_CUSTOM_HEADERS = `X-Osabio-Auth: ${proxyToken}`;
...
describe("Fresh proxy setup", () => {
  it("creates .claude/settings.local.json with ANTHROPIC_BASE_URL and ANTHROPIC_CUSTOM_HEADERS", () => {
    ...
    writeSettingsLocal(settingsPath, "https://osabio.example.com", "brp_abc123");

    // Then .claude/settings.local.json is created with correct keys
    expect(fs.existsSync(settingsPath)).toBe(true);

    const content = JSON.parse(fs.readFileSync(settingsPath, "utf-8"));
    expect(content.env.ANTHROPIC_BASE_URL).toBe("https://osabio.example.com/proxy/llm/anthropic");
    expect(content.env.ANTHROPIC_CUSTOM_HEADERS).toBe("X-Osabio-Auth: brp_abc123");
  });
});
...
describe("Merge with existing settings", () => {
  it("preserves existing non-Brain env vars and non-env config keys", () => {
    ...
    // When the proxy setup step runs
    writeSettingsLocal(settingsPath, "https://osabio.example.com", "brp_xyz789");

    // Then the env keys are merged (not overwritten)
    const result = JSON.parse(fs.readFileSync(settingsPath, "utf-8"));
    expect(result.env.ANTHROPIC_BASE_URL).toBe("https://osabio.example.com/proxy/llm/anthropic");
...
// ---------------------------------------------------------------------------
// Scenario: Re-running osabio init updates proxy token
// ---------------------------------------------------------------------------
describe("Re-run updates token in place", () => {
  it("replaces proxy token while preserving all other settings", () => {
    ...
    writeSettingsLocal(settingsPath, "https://osabio.example.com", "brp_old_token");
    ...
    // When the user runs osabio init again with a new token
    writeSettingsLocal(settingsPath, "https://osabio.example.com", "brp_new_token");
...
// ---------------------------------------------------------------------------
// Scenario: ~/.osabio/config.json proxy token storage
// ---------------------------------------------------------------------------
describe("Brain config proxy token storage", () => {
  it("stores proxy_token and proxy_token_expires_at in repo config entry", () => {
    const configPath = path.join(tmpDir, "config.json");

    // Given an existing ~/.osabio/config.json with a repo entry
    const existingConfig = {
      repos: {
        "/Users/priya/project": {
          server_url: "https://osabio.example.com",
          workspace_id: "ws-123",
    ...
    // When osabio init Step 7 stores the proxy token
    ...
    expect(entry.server_url).toBe("https://osabio.example.com");
...
describe("No fallback to direct Anthropic", () => {
  it("settings.local.json always points to Brain proxy, never direct Anthropic URL", () => {
    ...
    writeSettingsLocal(settingsPath, "https://osabio.example.com", "osp_token");
