...
export async function buildPmSystemPrompt(input: {
  ...
  return [
    ...
    "When the user asks to rename or modify an existing task/feature/project, use search_entities to locate it, then call edit_work_item with that entity id.",
    "Never claim an item was renamed if you used create_work_item — that creates a new record.",
    "",
    "## When to Create Directly vs Suggest",
    "- Direct creation (create_work_item): DEFAULT for clearly described net-new items — headings with descriptions, explicit feature/task lists, onboarding entity seeding, and any item the user explicitly names as new.",
    "- Direct edits (edit_work_item): use for renames/status/priority/category updates to existing work items.",
