...
where is the mcp endpoint implementation??

### Prompt 11

why haven't we removed the dead code and tests ? @app/src/server/orchestrator/spawn-agent.ts

### Prompt 12

DO NOT KEEP FUCKING LEGACY CODE

### Prompt 13

when the adapter is local for sandbox agent, then it should manage git worktrees ...

### Prompt 14

what about cleaning up the git worktrees?
...
Stop hook feedback:
Prompt hook condition was not met: Cannot use Brain MCP tools — not configured in this environment. Brain memory system (super-save, super-search) requires MCP server connection. Please run '/ctx:ctx-wrap-up' (built-in context tool) or save critical findings manually to project memory files.
