...
report issue in their repo for mock agent: https://github.com/rivet-dev/sandbox-agent

### Prompt 39

"For acceptance tests, we should use claude agent with the Brain proxy URL pointed at the real Anthropic API (since the test environment has ANTHROPIC_API_KEY available). But that uses real tokens…" this is fine. route through openrouter via env vars and use the cheap haiku model
...
"Implement a sliding window rate limiter with 100 requests per minute" i don't think we should give it an actual task that does stuff. it should be something simple like asking a question and expecting an answer. we don't want it messing randomly around with file system in a test...
