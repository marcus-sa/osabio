...
why are we recreating the intent handler from scratch? dont we already have this functionality implemented in the code base?

### Prompt 36

how do i enable the sandbox agent ?

### Prompt 37

.env has been configured with these env vars, but starting the server fails with:

552 |   // Resolve adapter: real SDK adapter from deps, or mock for tests
553 |   const sandboxAdapterPromise = wiringDeps.sandboxAgentAdapter
554 |     ? Promise.resolve(wiringDeps.sandboxAgentAdapter)
555 |     : wiringDeps.mockAgent
556 |       ? import("./sandbox-adapter").then((mod) => mod.createMockAdapter())
557 |       : Promise.reject(new Error("No sandbox adapter configured — set sandboxAgentAdapter o...
...
where is the mcp config set ?

### Prompt 40

well, we need to implement it, otherwise the agent doesnt have any mcp tools available...
...
https://sandboxagent.dev/docs/mcp-config#configuring-mcp-servers

### Prompt 43

well we want it to be per session not per server...
...
open a github issue for this in their repo https://github.com/rivet-dev/sandbox-agent
...
also since sandbox agent doesnt support per session mcp config, how tf would it work with cloud deployments like https://sandboxagent.dev/docs/deploy/e2b - that is also worth noting in the github issue
