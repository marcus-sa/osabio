...
add the chosen paradigm "Use functional paradigm" to CLAUDE.md so future delivers don’t ask again.
...
"No modals exist in the codebase. The simplest fix: convert the dead /settings link into an inline form that POSTs to the existing POST /api/workspaces/:ws/repo-path endpoint. Let me check the workspace API client."

create a separate github issue for this: we need to setup shadcn ui
